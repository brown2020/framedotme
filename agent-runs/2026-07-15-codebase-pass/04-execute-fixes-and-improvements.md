...
- Added explicit native/shared button semantics, stable FAQ keys, image `sizes`, non-redundant landmark markup, and exact provider-array length validation.
...
| Public surface area | Pass | Wrong/unused ID-token verifier and unused `buttonVariants` export removed; narrow APIs remain. | Continue F-108 proof-backed cleanup |
