...
| F-108 | P2 | Dead code/metadata | Open | Reachability | Unreferenced auth UI, legacy video playback, token-leader, env/type compatibility files, and unused selector/helper exports expand surface. Five route metadata modules are also unreachable, so intended metadata is not applied. | React Doctor deslop output plus zero external imports from repository-wide `rg` | Delete proven-isolated clusters; wire metadata through route layouts; remove only zero-use exports/dependencies; lint/build/Doctor. |
...
| F-110 | P2 | Reliability | Open | Firebase initialization | Firebase client/admin initialization catches failures and exports cast-empty SDK objects, deferring clear configuration errors into unrelated runtime calls. | `src/firebase/firebaseClient.ts`, `src/firebase/firebaseAdmin.ts` | Replace cast-empty fallbacks with explicit initialization failure while preserving build-time public env access; lint/build. |
| F-111 | P3 | Maintainability | Open | Zod/Fast Refresh | `z.string().url()` is deprecated in Zod 4; `buttonVariants` is an unused non-component export from a component file. | React Doctor warnings; `src/lib/validation.ts:43`, `src/components/ui/button.tsx:57` | Use `z.url()` and narrow button export. |
